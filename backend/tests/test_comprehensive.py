...
class TestAnomalyDetector:
    ...
    def test_rule_based_detection_flags_suspicious_ports(self):
        ...
        # Reasons are returned as "<reason>:<detail>" so the offending port is
        # visible to the analyst; assert on the prefix.
        assert any(a.startswith("suspicious_port:") for a in anomalies)
        assert any(a.startswith("known_malware_port:") for a in anomalies)
...
class TestSecurityAndAbuse:
    ...
    def test_cypher_injection_prevented(self):
        ...
        # Templates must be static text with no Python interpolation site, so
        # user input can only ever arrive as a bound $parameter.
        # Note: a bare `+` is NOT evidence of injection -- Cypher uses it for
        # its own string concatenation, e.g. "'Relationship: ' + rel_type".
        for intent, template in CYPHER_TEMPLATES.items():
            assert "%s" not in template, f"printf-style placeholder in {intent}"
            assert ".format(" not in template, f"str.format call in {intent}"
            assert not re.search(r"\{[a-zA-Z_]\w*\}", template), (
                f"format-brace placeholder in {intent}"
            )

    def test_malicious_entity_stays_a_parameter(self):
        """❌ A hostile entity value is bound, never spliced into the query."""
        from app.services.cypher_query_service import CypherQueryService, QueryIntent

        payload = "1.2.3.4' DETACH DELETE n //"
        calls = []

        class _Recorder:
            def query_graph(self, graph_id, cypher_query, params=None):
                calls.append((cypher_query, params or {}))
                return []

        service = CypherQueryService.__new__(CypherQueryService)
        service.neo4j_service = _Recorder()

        results, used_cypher = service.execute_query(
            "test_graph", QueryIntent.IP_CONNECTIONS, {"ip": payload}
        )

        assert calls, "expected the service to issue a query"
        # Every statement issued (EXPLAIN probe included) must keep the payload
        # out of the query text and pass it as a bound parameter instead.
        for cypher, params in calls:
            assert payload not in cypher, "payload was spliced into Cypher"
            assert "DETACH DELETE" not in cypher
            assert params.get("ip") == payload, "payload should be bound as $ip"

        assert payload not in used_cypher
