...
from app.models.network_models import (
    NetworkLogsInput, NetworkGraphResponse,
...
async def ingest_network_logs(
    input_data: NetworkLogsInput,
...
async def get_anomalies(
    ...
    try:
        ...
        # The OR must be parenthesised: without it Cypher binds AND tighter,
        # so `is_anomaly = true` alone would let Port nodes through.
        cypher_query = """
        ...
        # "normal" is derived from the total rather than counted here, because
        # this query only returns nodes already flagged as anomalous.
...
async def get_graph_correlations(
    graph_id: str,
    neo4j_service: Neo4jService = Depends(get_neo4j_service),
):
    """
...
async def cleanup_old_graphs(
    keep_network_security: bool = True,
    neo4j_service: Neo4jService = Depends(get_neo4j_service),
):
    """
