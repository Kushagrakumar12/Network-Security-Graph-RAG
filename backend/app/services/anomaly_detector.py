...
class NetworkAnomalyDetector:
    ...
    def _rule_based_detection(self, conn: Dict[str, Any]) -> List[str]:
        ...
        # Note: a destination port in the IANA dynamic range (49152-65535) is
        # deliberately NOT flagged here. That range carries ordinary client
        # source ports, passive-FTP data channels and plenty of legitimate
        # high-port services, so tagging it marked routine traffic as anomalous
        # and inflated every downstream anomaly score. The high ports that do
        # matter (31337, 54321, 12345, ...) are already covered by
        # SUSPICIOUS_PORTS via the suspicious_port tag above, and
        # _port_rarity_score still gives the model a continuous signal for
        # uncommon ports.
