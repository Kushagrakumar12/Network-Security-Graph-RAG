...
# Ports commonly used by backdoors, RATs and C2 frameworks.
# 4444 is the Metasploit/Meterpreter default, 31337 the classic Back Orifice
# "elite" port, and the repdigit/sequential ports are frequent hand-picked
# listener defaults.
...
def is_suspicious_port(port: int) -> bool:
    """
    Check if a port is known to be used by malware or C2 tooling.

    Only the curated SUSPICIOUS_PORTS list counts. Sitting in the IANA dynamic
    range (49152-65535) is deliberately NOT treated as suspicious: that range is
    where ordinary client source ports and many legitimate high-port services
    live, so flagging it marked most normal traffic as malicious and inflated
    every anomaly score downstream.
    """
