...
# Relationship types are interpolated into Cypher (Cypher has no portable
# dynamic relationship type), so they must match this pattern exactly.
_REL_TYPE_PATTERN = re.compile(r"^[A-Z][A-Z0-9_]*$")
DEFAULT_RELATIONSHIP_TYPE = "RELATED_TO"


def sanitize_relationship_type(label: Optional[str]) -> str:
    """
    Normalise an edge label into a safe Cypher relationship type.

    Returns DEFAULT_RELATIONSHIP_TYPE for anything that does not normalise to a
    bare uppercase identifier, so a hostile or malformed label can never be
    interpolated into a query.
    """
    if not label:
        return DEFAULT_RELATIONSHIP_TYPE

    candidate = re.sub(r"\s+", "_", str(label).strip()).upper()
    if not _REL_TYPE_PATTERN.match(candidate):
        logger.warning(
            f"Rejected unsafe relationship type {label!r}; "
            f"stored as {DEFAULT_RELATIONSHIP_TYPE}"
        )
        return DEFAULT_RELATIONSHIP_TYPE
    return candidate
...
class Neo4jService:
    ...
    def ensure_indexes(self):
        ...
        try:
            with self.driver.session() as session:
                ...
                # Every Cypher template in cypher_query_service filters nodes by
                # `type` (InternalIP / ExternalIP / Port / AttackType), so this
                # index backs the hottest predicate in the query layer.
    ...
    def _store_graph_internal(self, graph: Dict[str, Any], graph_id: str, batch_size: int, use_merge: bool) -> str:
        ...
        with self.driver.session() as session:
            ...
            # Group edges by relationship type so each edge keeps its real label.
            # Cypher has no portable dynamic relationship type, so the type is
            # validated against a strict pattern and interpolated per group.
            edges_by_type: Dict[str, List[Dict[str, Any]]] = {}
            for edge in graph["edges"]:
                edge_data = edge["data"]
                rel_type = sanitize_relationship_type(edge_data.get("label"))
            ...
            for rel_type, type_edges in edges_by_type.items():
                for i in range(0, len(type_edges), batch_size):
                    ...
                    # Keying the relationship on edge.id keeps every distinct
                    # connection between the same pair of nodes as its own
                    # relationship, so per-connection properties such as `port`
                    # survive. Without it, MERGE collapses them into one edge.
                    if use_merge:
                        cypher = f"""
                        ...
                        MATCH (source:Node {{id: edge.source}})
                        MATCH (target:Node {{id: edge.target}})
                        CREATE (source)-[r:{rel_type} {{id: edge.id}}]->(target)
                        SET r += edge.properties
                        """
    ...
    def execute_query(self, cypher_query: str, params: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Run a Cypher query that is not scoped to a single graph.

        Unlike query_graph, this does not inject a graph_id parameter, so it
        suits cross-graph lookups. Callers must still parameterise all user
        input rather than interpolating it into cypher_query.
        ...
        Returns:
            results: List of query results, or an empty list on failure.
        """
        start_time = time.time()
        if params is None:
            params = {}
