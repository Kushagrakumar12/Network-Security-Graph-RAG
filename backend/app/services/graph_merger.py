...
class GraphMerger:
    ...
    def merge_graphs(
        ...
        # Store the merged graph under merged_graph_id. store_graph() would mint
        # its own UUID and discard this id, so the id returned to the caller
        # would never match anything in Neo4j.
        stored_graph_id = self.neo4j_service.store_graph_merge(
            merged_graph, graph_id=merged_graph_id
        )

        logger.info(f"Merged graphs: {stats}")

        return {
            "merged_graph_id": stored_graph_id,
            "statistics": stats,
